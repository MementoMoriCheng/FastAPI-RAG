...
from app.rag.parser import DocxParser, ExcelParser, JsonParser, MarkdownParser, PdfParser, PlainParser, TxtParser
from app.rag.nlp import concat_img, find_codec, naive_merge, naive_merge_docx, rag_tokenizer, tokenize_chunks, \
    tokenize_chunks_docx, tokenize_table
...
def chunk(filename, binary=None, from_page=0, to_page=100000,
    ...
    if re.search(r"\.docx$", filename, re.IGNORECASE):
        ...
        if layout_recognizer == "DeepDOC":
            ...
            # if vision_model:
            #     sections, tables, figures = pdf_parser(filename if not binary else binary, from_page=from_page,
            #                                            to_page=to_page, callback=callback, separate_tables_figures=True)
...
if __name__ == "__main__":
    import sys
    from app.rag.utils.mongodb_conn import mongo
    ...
    mango_chunk=[]
    res = chunk(sys.argv[1], from_page=0, to_page=5, callback=dummy)
    for con in res:
        con.pop("image", None)
        mango_chunk.append(con)
    print(mango_chunk)
    mongo.insert_many(res)